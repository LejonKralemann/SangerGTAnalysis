...
for (i in row.names(Total_seq_rxns)) {
  ...
  for (j in row.names(sites)) {
    ...
    #if there is only 1 type of outcome, make that the final outcome
      }else if  (nrow(Subset_working_2) == 1){
      Consensus = Subset_working_2 %>% select(-no_observations)  
      #if there are 2 types of outcome
      }else if (nrow(Subset_working_2) == 2){
