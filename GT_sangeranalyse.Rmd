  ...
  for (j in row.names(sites)) {
    ...
    if (nrow(Subset_working_2) == 0) {
      ...
      names(Consensus)[names(Consensus) == "temp_j"] <- Site_name
        ...
        if ("wt" %in% Subset_working_2[,2]){
          if ("GT hom" %in% Subset_working_2[,2] | "GT het" %in% Subset_working_2[,2] | "GT hom (GT het)" %in% Subset_working_2[,2]){
            Consensus = data.frame(DNASample = {{Sample_name}}, temp_j = "undetermined")
            names(Consensus)[names(Consensus) == "temp_j"] <- Site_name
          }else{
        ...
        }else if ("GT hom" %in% Subset_working_2[,2]){
          if ("GT het" %in% Subset_working_2[,2]){
        ...
        }else if ("GT het" %in% Subset_working_2[,2]){
            Consensus = data.frame(DNASample = {{Sample_name}}, temp_j = "GT het")
            names(Consensus)[names(Consensus) == "temp_j"] <- Site_name
        }else if ("wt (GT het)" %in% Subset_working_2[,2]){
          if ("GT hom (GT het)" %in% Subset_working_2[,2]){
                        Consensus = data.frame(DNASample = {{Sample_name}}, temp_j = "GT het")
                        names(Consensus)[names(Consensus) == "temp_j"] <- Site_name
          }else{
            Consensus = data.frame(DNASample = {{Sample_name}}, temp_j = "undetermined")
