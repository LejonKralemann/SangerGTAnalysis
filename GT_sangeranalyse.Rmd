...
sites=read.xlsx("sanger_analysis_input/SangerAnalysisInput.xlsx", sheet="gt_sites")
dna_samples = read.xlsx("sanger_analysis_input/SangerAnalysisInput.xlsx", sheet="sangerseqno_dnasample")
reftable=read.xlsx("sanger_analysis_input/SangerAnalysisInput.xlsx", sheet="reference")
primerinfo=read.xlsx("sanger_analysis_input/SangerAnalysisInput.xlsx", sheet="primers")
parentinfo=read.xlsx("sanger_analysis_input/SangerAnalysisInput.xlsx", sheet="dnasample_parents")
...
saveWorkbook(work_book, file = "SangerSeqAnalysis.xlsx", overwrite=TRUE)
...
```{r}  
  ...
  #temp file reading
  Total_trans=read.xlsx("SangerSeqAnalysis.xlsx", sheet="Data_compact") %>% left_join(primerinfo) %>% filter(Orientation == "FW") %>% select(SangerSeqNo, Site_1, Site_2, Site_3, Site_4, Site_5, Site_6)

  #correct the interpretation. When you go from determined to undetermined, back to determined, it means there was likely a deletion, and you have reached the end of one of the two sequences. The second stretch of determined sites can no longer be assigned GT het or GT hom, but should be assigned GT. 
  
  #fw reactions:
  rm(Total_corrected)
  for (i in row.names(Total_trans)){
   Sanger_seq_name = as.character(Total_trans %>% filter(row.names(Total_trans) %in% i) %>% select(SangerSeqNo)) 
   Current_row = Total_trans %>% filter(SangerSeqNo == Sanger_seq_name) %>% select(-SangerSeqNo)
   call_zone = FALSE #when calling as determined
   second_garbage = FALSE #when calling as undetermined, but not at the beginning of the sequence
   rm(current_row_corrected)
   
    for (j in colnames(Current_row)){
      current_field = Current_row[[1,j]]
      if (current_field != "undetermined" & current_field != "mutated" & second_garbage == FALSE){
        call_zone = TRUE
        current_call = current_field
      }else if (call_zone == TRUE){
        second_garbage = TRUE
        call_zone = FALSE
        current_call = current_field
      }else if (current_field == "GT hom" | current_field == "GT het"){
          current_call = "GT hom/het"
      }else if (current_field == "wt"){
        current_call = "wt/GT het"
      }else{
          current_call = "undetermined"
      }
      if (exists("current_row_corrected")==TRUE){
        current_row_corrected = c(current_row_corrected, current_call)
      }else{
        current_row_corrected = current_call
      }
    }
   current_row_corrected_name = c(Sanger_seq_name, current_row_corrected)
   
   if (exists("Total_corrected")==TRUE){
   Total_corrected = rbind(Total_corrected, current_row_corrected_name)
   
   }else{
     Total_corrected = current_row_corrected_name
   }
  }
  
    Total_trans=read.xlsx("SangerSeqAnalysis.xlsx", sheet="Data_compact") %>% left_join(primerinfo) %>% filter(Orientation == "RV") %>% select(SangerSeqNo, Site_1, Site_2, Site_3, Site_4, Site_5, Site_6)
...
#combine the data from different reactions
```{r}
#give preference to GT het, GT hom and wt over undetermined/mutated. Mutated over undetermined. If multiples of gt het, hom or wt are present on a spot, take what the majority say (and if there are only 2, I should sequence more)
  ...
  for (j in row.names(sites)) {
    ...
    if (nrow(Subset_working_2) == 0) {
      ...
      if (nrow(Subset_working_2) > 1) {
        ...
        if (Top_hit_number == Second_hit_number) {
          #if one of the two is "GT"
          if (Top_hit[[1,2]] == "") {
            Consensus = data.frame(DNASample = {{Sample_name}}, temp_j = "conflict")
            names(Consensus)[names(Consensus) == "temp_j"] <- Site_name
            
          } else{
            #it becomes either GT hom or GT het
          }
        } else{
          Consensus = Top_hit %>% select(-no_observations)
        }
