...
plt=ggplot(data=alldata_combined3, aes(fill=fill_colour), colour="black", linewidth=line.thick)+
  ...
  geom_text(aes(y=y_pos_sample, x=7.5, label=DNASample, colour=Genotype), size=geom.text.size)+
  #vertical lines next to plot for F2 groups
  geom_segment(aes(y=ypos_F2_start-0.25, yend=ypos_F2_stop+0.25, x=8, xend=8))+
  #F2 names next to plot
  geom_text(aes(y=(ypos_F2_start+ypos_F2_stop)/2, x=8.5, label=F2, colour=Genotype), size=geom.text.size)+
  #vertical F1 group lines next to plot
  geom_segment(aes(y=ypos_F1_start-0.25, yend=ypos_F1_stop+0.25, x=9, xend=9))+
  #F1 names next to plot
  geom_text(aes(y=(ypos_F1_start+ypos_F1_stop)/2, x=9.5, label=F1), size=geom.text.size)+
  #vertical T2 group lines next to plot
  geom_segment(aes(y=ypos_T2_start-0.25, yend=ypos_T2_stop+0.25, x=10, xend=10))+
  #T2 names next to plot
  geom_text(aes(y=(ypos_T2_start+ypos_T2_stop)/2, x=10.5, label=T2), size=geom.text.size)+
  ...
  annotate("text", label="F3", y=-1, x=7.5, size=geom.text.size, hjust=0.5)+
  annotate("text", label="F2", y=-1, x=8.5, size=geom.text.size, hjust=0.5)+
  annotate("text", label="F1", y=-1, x=9.5, size=geom.text.size, hjust=0.5)+
  annotate("text", label="T2", y=-1, x=10.5, size=geom.text.size, hjust=0.5)+
...
plt=ggplot(data=alldata_combined3, aes(fill=fill_colour), colour="black", linewidth=line.thick)+
  ...
  geom_segment(aes(y=ypos_F1_start-0.25, yend=ypos_F1_stop+0.25, x=7, xend=7))+
  #F1 names below
  geom_text(aes(y=(ypos_F1_start+ypos_F1_stop)/2, x=7.5, label=F1), size=geom.text.size)+
  #horizontal lines below for T2 groups
  geom_segment(aes(y=ypos_T2_start-0.25, yend=ypos_T2_stop+0.25, x=8, xend=8))+
  #T2 names below
  geom_text(aes(y=(ypos_T2_start+ypos_T2_stop)/2, x=8.5, label=T2), size=geom.text.size)+
  ...
  geom_segment(aes(y=0.5, yend=max(alldata_ypos$y_pos_sample)+0.5, x=4.5, xend=4.5), colour="white", linetype="dashed")+
  #legend for info below plot
  annotate("text", label="F1", y=-1, x=7.5, size=geom.text.size, hjust=0.5)+
  annotate("text", label="T2", y=-1, x=8.5, size=geom.text.size, hjust=0.5)+
  #rectangle color legend
  #rectangle color legend
  scale_fill_identity(guide="legend", breaks=alldata_ypos_fill_legend$fill_colour, labels=alldata_ypos_fill_legend$value)+
  ...
  labs(x="Site", y="F2 individuals", fill="", shape="")+
    theme(panel.border = element_blank(), 
        ...
        legend.direction = "vertical",
        legend.text = element_markdown(size=theme.size))+
  coord_cartesian(xlim=c(-0, no_sites+2.5), expand=FALSE, clip="off")

plt
ggsave(file=paste0(output_dir, "main_stackedbar_sup.png"), width=14, height=10, units = "cm", dpi = "print")
...
plt
ggsave(file=paste0(output_dir, "ectopicGT_stackedbar.png"), plot=plt, width=14, height=5, units = "cm", dpi = "print")
...
alldata=read.xlsx(paste0(output_dir, "SangerSeqAnalysis.xlsx"), sheet="Data_summary") %>%
  ...
  filter(Site_1 != "mut/mut" & Site_2 != "mut/mut" & Site_3 != "mut/mut" & Site_4 != "mut/mut" & Site_5 != "mut/mut" & Site_6 != "mut/mut" )%>%
  filter(Site_1 != "wt/mut" & Site_2 != "wt/mut" & Site_3 != "wt/mut" & Site_4 != "wt/mut" & Site_5 != "wt/mut" & Site_6 != "wt/mut" )%>%
...
plt = ggplot(data=data_figure_combined, aes(y=rown))+
  geom_col(aes(x=mean_downlen), fill="grey90", col="black", orientation = "y")+
  geom_col(aes(x=mean_uplen), fill="grey90", col="black", orientation = "y")+
  labs(x="Mean gene conversion tract length (bp from DSB)", y="F3s grouped by F2 parent")+
    theme(
        ...
        legend.direction = "vertical",
        legend.text = element_markdown(size=theme.size))+
  geom_vline(xintercept=-693, lty="dotted", col="red")+
  geom_vline(xintercept=118, lty="dotted", col="red")+
  geom_errorbar(aes(xmin=SE_up, xmax = mean_uplen))+
  geom_errorbar(aes(xmin=mean_downlen, xmax = SE_down))+
