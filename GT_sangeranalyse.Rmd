...
plt=ggplot(data=alldata_combined4, aes(fill=fill_colour), colour="black", linewidth=line.thick)+
  ...
  #horizontal lines below plot for F2 groups
  geom_segment(aes(y=ypos_F2_start-0.25, yend=ypos_F2_stop+0.25, x=8.5, xend=8.5))+
  #F2 names below plot
  geom_text(aes(y=(ypos_F2_start+ypos_F2_stop)/2, x=9, label=F2, colour=Genotype), size=geom.text.size)+
  #horizontal F1 group lines below plot
  geom_segment(aes(y=ypos_F1_start-0.25, yend=ypos_F1_stop+0.25, x=9.5, xend=9.5))+
  #F1 names below plot
  geom_text(aes(y=(ypos_F1_start+ypos_F1_stop)/2, x=10, label=F1), size=geom.text.size)+
  #horizontal T2 group lines below plot
  geom_segment(aes(y=ypos_T2_start-0.25, yend=ypos_T2_stop+0.25, x=10.5, xend=10.5))+
  #T2 names below plot
  geom_text(aes(y=(ypos_T2_start+ypos_T2_stop)/2, x=11, label=T2), size=geom.text.size)+
  ...
  annotate("text", label="Del.", y=-1, x=8, size=geom.text.size, hjust=0.5)+
  annotate("text", label="F2", y=-1, x=9, size=geom.text.size, hjust=0.5)+
  annotate("text", label="F1", y=-1, x=10, size=geom.text.size, hjust=0.5)+
  annotate("text", label="T2", y=-1, x=11, size=geom.text.size, hjust=0.5)+
  ...
  #draw a circle to deletions
  geom_point(aes(y=y_pos_sample, x=8, shape=deletion_shape), size=1)+

  coord_cartesian(xlim=c(-0, no_sites+5.5), expand=FALSE, clip="off")

plt
ggsave(file=paste0(output_dir, "main_stackedbar.png"), plot=plt, width=5, height=11)
...
plt=ggplot(data=alldata_combined4, aes(fill=fill_colour), colour="black", linewidth=line.thick)+
  ...
  geom_segment(aes(y=ypos_F1_start-0.25, yend=ypos_F1_stop+0.25, x=8.5, xend=8.5))+
  #F1 names below
  geom_text(aes(y=(ypos_F1_start+ypos_F1_stop)/2, x=9, label=F1), size=geom.text.size)+
  #horizontal lines below for T2 groups
  geom_segment(aes(y=ypos_T2_start-0.25, yend=ypos_T2_stop+0.25, x=9.5, xend=9.5))+
  #T2 names below
  geom_text(aes(y=(ypos_T2_start+ypos_T2_stop)/2, x=10, label=T2), size=geom.text.size)+
  #horizontal lines separating the sites
  geom_segment(aes(y=0.5, yend=max(alldata_ypos$y_pos_sample)+0.5, x=0.5, xend=0.5))+
  geom_segment(aes(y=0.5, yend=max(alldata_ypos$y_pos_sample)+0.5, x=1.5, xend=1.5))+
  geom_segment(aes(y=0.5, yend=max(alldata_ypos$y_pos_sample)+0.5, x=2.5, xend=2.5))+
  ...
  annotate("text", label="Ect.", y=-1, x=7, size=geom.text.size, hjust=0.5)+
  annotate("text", label="Del.", y=-1, x=8, size=geom.text.size, hjust=0.5)+
  annotate("text", label="F1", y=-1, x=9, size=geom.text.size, hjust=0.5)+
  annotate("text", label="T2", y=-1, x=10, size=geom.text.size, hjust=0.5)+
  #rectangle color legend
  #rectangle color legend
  scale_fill_identity(guide="legend", breaks=alldata_ypos_fill_legend$fill_colour, labels=alldata_ypos_fill_legend$value)+
  #scale_color_manual(name="F2 bkgd:", breaks=c("wt", "teb"), labels=c(tebwt, tebmut), values=c("black", "red"))+
